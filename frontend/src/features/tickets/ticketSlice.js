...
export const createTicket = createAsyncThunk(
    ...
    async (ticketData, thunkAPI) => {
        try {
            ...
            return await ticketService.createTicketService(ticketData, token)
        } catch (error) {
            const message =
...
export const getTickets = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            const token = thunkAPI.getState().authR.user.token
            return await ticketService.getTicketsService(token)
        } catch (error) {
            const message =
                error.response?.data?.message ||
                error.message ||
...
export const getTicket = createAsyncThunk(
    ...
    async (ticketId, thunkAPI) => {
        try {
            const token = thunkAPI.getState().authR.user.token
            return await ticketService.getTicketService(ticketId, token)
        } catch (error) {
            const message =
                error.response?.data?.message ||
                error.message ||
